...
for label, needle in required_main.items():
    if needle not in main:
        raise SystemExit(f"v4.25 verification failed: missing {label}: {needle}")
...
if 'versionCode = 50' not in gradle or 'versionName = "4.25"' not in gradle:
    raise SystemExit("v4.25 verification failed: app version is not 4.25 / code 50")

# The v4.24 whole-content Left fallback caused poster-grid Left presses to
# bubble out to the category rail. v4.25 must remove that broad handler.
...
if bad_fallback in main:
    raise SystemExit("v4.25 verification failed: broad Home content Left fallback is still present")
...
if player_builders != 1:
    raise SystemExit(f"v4.25 verification failed: expected one ExoPlayer.Builder, found {player_builders}")

if "EpgStore.guide" not in main:
    raise SystemExit("v4.25 verification failed: guide UI is not using EpgStore")

print("Zako v4.25 source verification passed")
